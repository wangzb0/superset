...
export default function EmptyState({ tableName, tab }: EmptyStateProps) {
  ...
  const mine = (
    ...
    /*<span>
      {t('您还没有%(tableName)s', { tableName: welcomeTableLabels[tableName] })}
    </span>*/
  );
  const recent = (
      ...
      {(() => {
        ...
        if (tab === 'Examples') {
          return t('Example %(tableName)s will appear here', {
            // tableName: tableName.toLowerCase() === 'dashboards' ? '看板' : '图表'
            tableName: function() {
              if (tableName.toLowerCase() === 'dashboards') {
                return '看板';
              } else if (tableName.toLowerCase() === 'charts') {
                return '图表';
              } else {
